...
class ChatRepository(private val chatDao: ChatDao) {
    ...
    suspend fun ensureSession(sessionId: String?, titleHint: String = "Prediksi baru"): SessionEntity {
    ...
    suspend fun renameSession(sessionId: String, title: String) {
        ...
        val session = existing?.copy(title = title, updatedAt = now)
            ?: SessionEntity(id = sessionId, title = title, createdAt = now, updatedAt = now)
        chatDao.upsertSession(session)
    }
